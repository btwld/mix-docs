...
<Callout type="info">
  **Prerequisites:** This is an advanced guide. You should be familiar with [Styling](/documentation/mix/guides/styling) and [Design Tokens](/documentation/mix/guides/design-token) before reading this.
</Callout>
...
StyledText('hello world', style: TextStyler().uppercase())
```
...
```dart
final style = BoxStyler().color(Colors.blue.withOpacity(0.5));

// Color directives also work with tokens
final style = BoxStyler().color($primary().withOpacity(0.8));
```
...
extension ReverseTextDirective on TextStyler {
  TextStyler reverse() {
    return merge(
      .textDirective(const ReverseStringDirective()),
    );
  }
}
...
- [Design Tokens](/documentation/mix/guides/design-token) — tokens work with directives via `Prop.token()`
- [Widget Modifiers](/documentation/mix/guides/widget-modifiers) — widget wrappers (modifiers transform widgets, directives transform values)
