...
This guide builds on the [Styling](/documentation/mix/guides/styling) guide. If you haven't read it yet, start there to learn how `Style` and `Styler` work.
...
final style = BoxStyler()
    ...
    .color(
      Colors.green,
    ); // This overrides the base color, but NOT the hover color
```
...
final styleA = BoxStyler().color(Colors.red).onHovered(.color(Colors.blue));
...
final style = BoxStyler()
    ...
    .onNot(.brightness(.dark), .color(Colors.white));
```
...
```dart
final style = BoxStyler().color(Colors.grey).onBuilder((context) {
...
**Manual state control:** The variants on this page are applied automatically. When you need programmatic control — toggling `selected`, sharing state across widgets, or custom gesture handling — see [Advanced Widget State Control](/documentation/mix/tutorials/controlling-widget-state).

**Custom context variants:** You can create custom `ContextVariant` instances that respond to any condition from `BuildContext` — such as an `InheritedWidget`, a feature flag, or app-specific state. See [Creating a Custom Context Variant](/documentation/mix/tutorials/creating-context-variants) for a step-by-step walkthrough.
