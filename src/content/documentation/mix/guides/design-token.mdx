...
Sometimes a token's value comes straight from the surrounding Flutter context — `Theme.of(context).colorScheme.primary`, the current `MediaQuery`, or a `Localizations` lookup. Wiring those through `MixScope` means recomputing the value at the right spot in the tree and re-providing it whenever it changes.

`ContextToken<T>` removes that step. You give it a resolver function that reads from `BuildContext`, and it computes its value at resolution time — **no `MixScope` required**.

```dart
// Declare once — the resolver runs against the consuming widget's context
final $primary = ContextToken<Color>(
  (context) => Theme.of(context).colorScheme.primary,
);

// Use it like any other token — no MixScope needed
final style = BoxStyler().color($primary());

Box(style: style); // resolves to the ambient Theme's primary color
```
...
| Best for              | Design-system values you swap by theme    | Values derived from Flutter context (`Theme`, `MediaQuery`, …) |
