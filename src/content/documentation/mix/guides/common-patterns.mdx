...
description: "Copy ready-made Mix patterns for buttons, layouts, tokens, and responsive styling"
---
...
PressableBox(
  ...
  style: .padding(.horizontal(24).vertical(12))
...
<Callout type="info">
  `PressableBox` = `Pressable` + `Box`. If you need interaction tracking around another widget instead of a `Box`, use just `Pressable` directly. See the [Pressable reference](/documentation/mix/widgets/pressable).
</Callout>
...
final title = TextStyler().fontSize(24).fontWeight(.bold).color(Colors.black);
...
Widget build(BuildContext context) {
  return card(child: title('Welcome back'));
}
```
...
final outlineButton = baseButton
    ...
    .border(.color(Colors.blue).width(1.5))
...
<Callout type="info">
  The `$` prefix is a naming convention, not a language feature. It signals "this is a token that resolves at build time" rather than a concrete value. See the [Design Tokens guide](/documentation/mix/guides/design-token) for custom token types, theme switching patterns, and programmatic resolution.
</Callout>
...
final containerStyle = BoxStyler()
    ...
    .onTablet(.padding(.all(24)).width(720))
    .onDesktop(.padding(.all(32)).width(960));
...
- [Styling](/documentation/mix/guides/styling) — the Styler pattern and fluent chaining
- [Dynamic Styling](/documentation/mix/guides/dynamic-styling) — variants for interaction, theme, and context
- [Design Tokens](/documentation/mix/guides/design-token) — tokens, `MixScope`, and theming
- [Animations](/documentation/mix/guides/animations) — implicit, phase, and keyframe animations
- [Widgets](/documentation/mix/widgets/stylewidgets) — full widget reference
