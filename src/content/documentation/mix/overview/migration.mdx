...
```dart
// DEPRECATED - v1.x
final style = Style($box.color.blue(), $box.padding(16), $box.borderRadius(8));
...
// v1.x
MixTheme(
  data: MixThemeData(colors: {primaryColor: Colors.blue}),
  child: MyApp(),
);

// v2.0
MixScope(colors: {primaryColor: Colors.blue}, child: MyApp());
```
...
Another important change is that `Style` no longer takes a generic type parameter, so it is no longer possible to constrain a style to a specific attribute type.
...
The order of modifiers was previously determined by their addition sequence. Now, we enforce a specific ordering but also allow customizable orders. This might cause behavior changes in rare cases. For details, refer to the [modifiers guide](/documentation/mix/guides/widget-modifiers).

### StyledWidgets updates

#### The new PressableBox

To provide a more consistent way to use the gesture APIs, we developed a new widget that merges the functionalities of `Box` and `Pressable`. Thus, the new `PressableBox` can receive a style like a `Box` and also accept interactions and its variants, similar to a `Pressable`. For more information, see the [PressableBox guide](/documentation/mix/widgets/pressable).
