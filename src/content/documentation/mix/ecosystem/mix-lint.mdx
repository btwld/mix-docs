...
final style = BoxStyler().color(primary()).borderRadius(.topLeft(rounded()));
```
...
MixScope(colors: {primary: Colors.blue}, child: child);
```
...
final style = layout.merge(appearance).onHovered(hovered);
```
