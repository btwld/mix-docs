...
description: "Create custom Mix design tokens for value types the built-in tokens don't cover"
---
...
<Callout type="info">
  If your value is **derived from `BuildContext`** — most commonly when **bridging Material's theme** (`Theme.of(context).colorScheme` / `textTheme`), but also `MediaQuery` or `Localizations` **when the resolved type is one of Mix's [built-in token value types](/documentation/mix/guides/design-token#built-in-token-types)** — you may not need a custom token. Use the built-in [`ContextToken`](/documentation/mix/guides/design-token#context-tokens), which runs a resolver against the consuming widget's context with no `MixScope` required. For unsupported types, use a custom token if you need to pass the value through style APIs, or resolve it programmatically with `.resolve(context)`.
</Callout>
