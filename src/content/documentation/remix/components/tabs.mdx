...
## Generated Fortal components

Remix includes generated Fortal components for the tab bar, tabs, and views:

<CodeGroup title="Generated Fortal components" defaultLanguage="dart">
```dart
import 'package:flutter/material.dart';
import 'package:remix/remix.dart';
    ...
    return RemixTabs(
      ...
      child: Column(
        children: [
          ...
          FortalTabView(tabId: 'tab1', child: Text('Content 1')),
          FortalTabView(tabId: 'tab2', child: Text('Content 2')),
        ],
      ),
    );
  }
...
<Info>
  See the [Fortal tabs styles source code](https://github.com/btwld/remix/blob/main/lib/src/components/tabs/fortal_tabs_styles.dart) for all available options.
</Info>
...
| `child` | `Widget` | **Required**. The tabs content. |
| `controller` | `NakedTabController?` | Optional. Optional controller for managing tab state. |
| `selectedTabId` | `String?` | Optional. The identifier of the currently selected tab. |
| `onChanged` | `ValueChanged<String>?` | Optional. Called when the selected tab changes. |
| `orientation` | `Axis` | Optional. The tab list orientation. |
| `enabled` | `bool` | Optional. Whether the tabs are enabled. |
| `onEscapePressed` | `VoidCallback?` | Optional. Called when Escape is pressed while a tab has focus. |
...
| `container(FlexBoxStyler value)` | Configures the container flex box style. |
