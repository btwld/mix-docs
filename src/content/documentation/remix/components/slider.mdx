...
<Info>
  See the [fortalSliderStyler source code](https://github.com/btwld/remix/blob/main/lib/src/components/slider/fortal_slider_styles.dart) for all available options.
</Info>
...
| `onChanged` | `ValueChanged<double>?` | Optional. Called during drag with the new value. When null, the slider is disabled. |
...
| `style` | `RemixSliderStyler` | Optional. The style configuration for the slider. |
...
| `enableFeedback` | `bool` | Optional. Whether to provide haptic feedback during value changes. Defaults to true. |
