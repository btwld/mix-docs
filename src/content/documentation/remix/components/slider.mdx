...
class _SliderExampleState extends State<SliderExample> {
  ...
  RemixSliderStyler get style {
    return RemixSliderStyler()
        .thumbSize(const .square(24))
        .thumb(
          .shape(
            .circle(),
          ).shadow(.color(Colors.black45).blurRadius(4).offset(x: 0, y: 2)),
        )
        ...
        .onDisabled(
          .trackColor(Colors.grey.shade300)
              ...
              .thumb(.color(Colors.white.withValues(alpha: 0.6))),
        );
  }
}
```
...
<Info>
  See the [fortalSliderStyler source code](https://github.com/btwld/remix/blob/main/packages/remix/lib/src/components/slider/fortal_slider_styles.dart) for all available options.
</Info>
