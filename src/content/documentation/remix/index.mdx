...
Building fully custom UI in Flutter usually means starting from raw widgets — reimplementing hover, focus, and press handling for every component — or adapting a design language like Material to a look it wasn't designed for. Common friction points:

- **Scattered styling**: without a dedicated styling layer, visual decisions spread across build methods and are hard to keep consistent.
...
Build a blue button labeled "Click Me". The `RemixButtonStyler` class defines styles with a fluent API, and calling it directly produces the widget:
...
class Example extends StatelessWidget {
  ...
  RemixButtonStyler get _button => RemixButtonStyler()
      .paddingX(16)
      .paddingY(10)
      .color(Colors.blue)
      .borderRadius(.circular(8))
  ...
  Widget build(BuildContext context) {
    return _button(label: 'Click Me', onPressed: () {});
  }
}
```
...
final primaryButton = baseButton.color(Colors.blue).label(.color(Colors.white));
...
```dart
FortalButton.solid(onPressed: () {}, label: 'Fortal Button');
```
