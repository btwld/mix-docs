---
title: Fortal
description: Fortal is Remix's prebuilt design system with complete component styles, tokens, variants, and dark mode
---
...
Each Fortal widget delegates to a regular Remix `fortal*Styler` underneath. Those stylers use the same fluent API you'd use to write your own, so you can still extend, override, or fork any piece of the design system.
...
Then wrap the part of your app that should use Fortal in a `FortalScope`. The scope provides the token values that every Fortal widget, `fortal*Styler`, and `FortalTokens.*` call resolves against:
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: FortalScope(
        child: Scaffold(
          body: Center(
            child: FortalButton.solid(onPressed: () {}, label: 'Fortal Button'),
          ),
        ),
      ),
    );
...
Most Remix components expose generated `Fortal*` widgets with named constructors for fixed variants:

```dart
FortalButton.solid(label: 'Save', onPressed: () {})
FortalButton.soft(label: 'Save', onPressed: () {})
FortalButton.surface(label: 'Save', onPressed: () {})
FortalButton.outline(label: 'Save', onPressed: () {})
FortalButton.ghost(label: 'Save', onPressed: () {})
```

Use the unnamed constructor when the variant is selected dynamically:

```dart
FortalButton(variant: selectedVariant, onPressed: () {}, label: 'Save');
```

Call the matching `fortal*Styler` directly when you need a custom Remix widget composition or want to extend the preset style.
...
final style = fortalButtonStyler(
  variant: .solid,
).borderRadius(.circular(8)).paddingX(32).onHovered(.scale(1.05));
```
...
final style = RemixButtonStyler()
    ...
    .paddingAll(FortalTokens.space4())
    .borderRadius(.all(FortalTokens.radius3()))
    .onHovered(.color(FortalTokens.accent10()));
```
