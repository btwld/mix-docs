...
For buttons, use `ButtonStyler`. `RemixButtonStyler` remains available as a
deprecated compatibility alias during the beta releases.
...
```dart
final style = RemixMenuStyler.trigger(.color(Colors.black));
```
...
```dart
final style = RemixCardStyler.container(
  .new(transform: matrix, transformAlignment: .centerStart),
);
```
