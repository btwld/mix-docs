...
test('features The Production Gap from both the homepage and Reports page', () => {
  ...
  const reportData = readFileSync(join(root, 'components/reports/report-data.ts'), 'utf8')
  ...
  assert.ok(reportData.includes(reportUrl))
  assert.match(reportCard, /PRODUCTION_GAP_URL/)
  assert.match(reportCard, /Read the report/)
  assert.match(embeddedReport, /<iframe/)
  assert.match(embeddedReport, /src=\{PRODUCTION_GAP_URL\}/)
  assert.match(embeddedReport, /sandbox="allow-forms allow-popups allow-popups-to-escape-sandbox allow-same-origin allow-scripts"/)
})
